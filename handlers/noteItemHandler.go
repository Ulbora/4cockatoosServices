...
func (h *C2Handler) AddNoteItem(w http.ResponseWriter, r *http.Request) {
	...
	h.Log.Debug("AddNoteItem authorized: ", auth)
	h.SetContentType(w)
	if auth {
...
func (h *C2Handler) UpdateNoteItem(w http.ResponseWriter, r *http.Request) {
	auth := h.processAPIKeySecurity(r)
	h.Log.Debug("UpdateNoteItem authorized: ", auth)
	h.SetContentType(w)
	if auth {
		ucOk := h.CheckContent(r)
		h.Log.Debug("conOk: ", ucOk)
		if !ucOk {
			...
			if !suc && err != nil {
				...
				res := h.Manager.UpdateNoteItem(&req)
				h.Log.Debug("res: ", *res)
				if res.Success {
					w.WriteHeader(http.StatusOK)
				} else {
...
func (h *C2Handler) DeleteNoteItem(w http.ResponseWriter, r *http.Request) {
	auth := h.processAPIKeySecurity(r)
	h.Log.Debug("DeleteNoteItem authorized: ", auth)
	h.SetContentType(w)
	if auth {
		vars := mux.Vars(r)
		...
		if vars != nil && len(vars) == 1 {
			...
			var res *m.Response
			if iderr == nil {
				res = h.Manager.DeleteNoteItem(id)
				h.Log.Debug("res: ", res)
				if res.Success {
					w.WriteHeader(http.StatusOK)
				} else {
